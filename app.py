...
def clear_all_cache():
    ...
    load_other_cloud.clear()
    load_advance_cloud.clear()
    load_withdrawals_cloud.clear()
    load_withdrawal_sources_cloud.clear()
...
def clean_month(df: pd.DataFrame):
    df = df.copy()
...
def enrich_year_month(records):
    ...
    for r in records:
        ...
        nr = dict(r)
        nr["年份"] = d_parsed.year
        nr["月份"] = d_parsed.month
        nr["日期"] = d_parsed.isoformat()
        nr["付款人"] = str(nr.get("付款人", "") or "").strip()
        nr["發票"] = normalize_bool(nr.get("發票", False))
        nr["提領過"] = normalize_bool(nr.get("提領過", False))
        nr["金額"] = normalize_amount(nr.get("金額", 0))
        out.append(nr)
    return out
...
def create_withdrawal(w_date, amount, account, selected_uids, note=""):
    sources = []
    sum_amount = 0.0
    ...
    if abs(sum_amount - amount) > 0.01:
        st.error(f"選取來源總額 {sum_amount:.2f} 與提領金額 {amount:.2f} 不一致")
        return False
    ...
    for uid in selected_uids:
        rec, cat = get_expense_by_uid(uid)
        if cat == "固定":
            supabase.table("expense_fixed").update({"withdrawn": True}).eq("uid", uid).execute()
        elif cat == "其他":
            supabase.table("expense_other").update({"withdrawn": True}).eq("uid", uid).execute()
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("💾 儲存收入", key="save_income"):
    ...
    with c2:
        if st.button("🗑️ 刪除已勾選收入", key="delete_income"):
    ...
    with st.form("income_form"):
        ...
        with c1:
            year_income = st.selectbox("年", list(range(2021, 2051)), index=list(range(2021, 2051)).index(today.year), key="income_add_year")
        with c2:
            month_income = st.selectbox("月", list(range(1, 13)), index=today.month - 1, key="income_add_month")
        with c3:
            account = st.selectbox("帳戶", ["收錢吧", "美團", "抖音", "其他", "收錢吧-私"], key="income_add_account")
        with c4:
            amount_income = st.number_input("金額", min_value=0.0, key="income_add_amount")
        submitted_income = st.form_submit_button("新增收入")

    if submitted_income:
        insert_income_cloud(year_income, month_income, account, amount_income)
        st.rerun()

    with st.form("income_bulk_form"):
        st.markdown("### 📥 Excel貼上（收入）")
        text_income = st.text_area("格式：年份\t月份\t帳戶\t金額", key="ta_income")
        submit_income_bulk = st.form_submit_button("匯入收入")

    if submit_income_bulk:
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("💾 儲存固定支出", key="save_fixed"):
            ...
            locked_delete = [r for r in load_fixed_cloud() if r.get("uid") in deleting_uids and normalize_bool(r.get("提領過", False))]
    ...
    with c2:
        if st.button("🗑️ 刪除已勾選固定支出", key="delete_fixed"):
    ...
    with st.form("fixed_form"):
        ...
        submitted_fixed = st.form_submit_button("新增固定支出")
    ...
    with st.form("fixed_bulk_form"):
        ...
        submit_fixed_bulk = st.form_submit_button("匯入固定支出")

    if submit_fixed_bulk:
    ...
    o1, o2 = st.columns(2)
    with o1:
        if st.button("💾 儲存其他支出", key="save_other"):
    ...
    with o2:
        if st.button("🗑️ 刪除已勾選其他支出", key="delete_other"):
    ...
    with st.form("other_form"):
        ...
        submitted_other = st.form_submit_button("新增其他支出")
    ...
    with st.form("other_bulk_form"):
        ...
        submit_other_bulk = st.form_submit_button("匯入其他支出")

    if submit_other_bulk:
    ...
    with st.form("advance_form"):
        ...
        submitted_advance = st.form_submit_button("新增代墊支出")

    if submitted_advance:
        rec = {
    ...
    with st.form("advance_bulk_form"):
        st.markdown("### 📥 Excel貼上（代墊支出）")
        st.markdown(
            "推薦格式（Tab 分隔）：\n"
            "- 日期\t項目\t金額\t付款人\t發票\n"
            "也支援：\n"
        ...
        text_adv_bulk = st.text_area("在此貼上（代墊支出）", key="ta_advance")
        submit_adv_bulk = st.form_submit_button("匯入代墊支出")

    if submit_adv_bulk:
        recs = parse_other_bulk(text_adv_bulk)
        for r in recs:
            r["uid"] = next_uid("A")
            r.pop("提領過", None)

        current = load_advance_cloud()
        merged = current + recs
        replace_all_advance_cloud(merged)
        st.rerun()
...
with tab2:
    ...
    if not withdrawals_cloud:
        ...
        if st.button("🗑️ 刪除已勾選提領紀錄", key="delete_withdrawals"):
    ...
    with st.form("withdraw_form"):
        ...
        with c4:
            w_note = st.text_input("備註", key="w_note")
        ...
        if df_eligible.empty:
            ...
            selected_total = edited_eligible[edited_eligible["uid"].isin(selected_uids)]["金額"].sum() if selected_uids else 0.0
            st.write(f"已選取 {len(selected_uids)} 筆，總額：{selected_total:,.2f}")

        submit_withdraw = st.form_submit_button("建立提領紀錄")
...
with tab3:
    ...
    df_fixed = df_fixed.drop(columns=["id", "uid"], errors="ignore")
    ...
    if df_income.empty:
        ...
        if df_income_chart.empty:
            ...
            grouped["占比"] = grouped.apply(lambda r: 0 if r["月總收入"] == 0 else r["金額"] / r["月總收入"], axis=1)
            grouped["標籤"] = grouped.apply(lambda r: f'{r["金額"]:,.0f} ({r["占比"]:.0%})', axis=1)
    ...
    if monthly_summary.empty:
        ...
        combined_chart = (bar_chart + line_profit + line_acc).resolve_scale(y="shared").properties(height=500)
